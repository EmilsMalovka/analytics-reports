...
namespace EmilsMalovka\AnalyticsReports;
...
class AnalyticsReports
{
    ...
    {
        $visitorData = [];
        $answer = $this->performQuery($startDate, $endDate, 'ga:visits,ga:pageviews,ga:users,ga:newUsers,ga:percentNewSessions,ga:bounceRate,ga:avgSessionDuration', ['dimensions' => 'ga:'.$groupBy]);
        ...
        foreach ($answer->rows as $dateRow) {
            $visitorData[] = [
                ...
                'users' => $dateRow[3],
                'newUsers' => $dateRow[4],
                'precentNewSessions' => $dateRow[5],
                'bounceRate' => $dateRow[6],
                'avgSessionDuration' => $dateRow[7]
            ];
        }
    ...
    {
        return 'EmilsMalovka.analyticsReports.'.md5(serialize($properties));
    }
